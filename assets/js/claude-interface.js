...
function initSermonSearchInterface() {
    ...
    // Initialize example questions with proper translations
    initializeExampleQuestions();
    
    // Improve textarea handling
    improveTextareaHandling();
...
function restructureDOM() {
    ...
    if (!appContainer) {
        ...
        if (claudeInterface) {
            // Check if we need to wrap the interface in the chat-section-wrapper
            if (!claudeInterface.parentElement.classList.contains('chat-section-wrapper')) {
                const chatWrapper = document.createElement('div');
                chatWrapper.className = 'chat-section-wrapper';
                claudeInterface.parentElement.insertBefore(chatWrapper, claudeInterface);
                chatWrapper.appendChild(claudeInterface);
            }
            
            appContainer.appendChild(appHeader);
            appContainer.appendChild(claudeInterface.parentElement);
        } else {
...
/**
* Enhanced translation and example question handling 
*/

// Fix for example questions translation
function initializeExampleQuestions() {
console.log('Initializing example questions with proper translations');

// Get all example questions
const exampleItems = document.querySelectorAll('.example-questions li[data-i18n]');

if (exampleItems.length > 0) {
    console.log(`Found ${exampleItems.length} example questions to initialize`);
    
    // Update each item with its proper translation
    exampleItems.forEach(item => {
        const key = item.getAttribute('data-i18n');
        
        // Store the original text if needed for reference
        ...
        const translation = window.translations && 
                            window.translations[window.currentLanguage || 'en'] && 
                            window.translations[window.currentLanguage || 'en'][key];
        
        if (translation && !translation.startsWith('example-')) {
    ...
    console.warn('No example questions found for initialization');
}
}

// Enhanced translation function that properly handles data-i18n attributes
function enhancedTranslate(key) {
...
// Override the existing changeLanguage function
const originalChangeLanguage = window.changeLanguage;
window.changeLanguage = function(language) {
console.log(`Enhanced language change to: ${language}`);
...
if (typeof originalChangeLanguage === 'function') {
    ...
    document.querySelectorAll('[data-i18n]').forEach(element => {
        ...
        const translation = enhancedTranslate(key);
...
// Always update example questions explicitly
initializeExampleQuestions();
}
...
// Enhanced auto-resize textarea with improved behavior
function improveTextareaHandling() {
const queryInput = document.getElementById('queryInput');
...
if (queryInput.parentNode) {
    queryInput.parentNode.replaceChild(newQueryInput, queryInput);
}

// Add improved event listeners
newQueryInput.addEventListener('input', function() {
    adjustTextareaHeight(this);
});

// Better keyboard handling for form submission
newQueryInput.addEventListener('keydown', function(e) {
    // Submit on Enter (without Shift)
    if (e.key === 'Enter' && !e.shiftKey) {
        e.preventDefault();
        const chatForm = document.getElementById('chatForm');
...
// Add focus and blur handlers for visual feedback
newQueryInput.addEventListener('focus', function() {
    this.parentElement.classList.add('focused');
});

newQueryInput.addEventListener('blur', function() {
    this.parentElement.classList.remove('focused');
});
}
...
if (isMobile) {
    ...
    // Improve sources panel slide-up behavior
    const sourcesPanel = document.getElementById('sourcesPanel');
